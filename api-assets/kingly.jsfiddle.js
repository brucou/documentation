...
t.fsmContracts = Lt, t.build_state_enum = function(t) {
  ...
  return customElements.define(t, class extends HTMLElement {
    constructor() {
      if (t.split("-").length <= 1) throw "makewebcomponentfromfsm : web component's name must include a dash! please review the property passed as parameter to function!"; super(); const { subjectfactory: r }="e;" this.eventsubject="r()," this.options="Object.assign({}," i); c="this.options.NO_ACTION" || d; this.eventsubject.subscribe({ next: t> {
          const e = n(t);
          e !== c && e.forEach(t => {
            if (t === c) return;
            const {
